...
class Employee(
        ...
        @OneToMany(mappedBy = "coordinatorEmployee", cascade = [CascadeType.ALL], orphanRemoval = true, fetch = FetchType.LAZY)
        var employeeContracts: MutableList<Contract> = mutableListOf(),

        @OneToMany(mappedBy = "coordinatorEmployee", cascade = [CascadeType.ALL], orphanRemoval = true, fetch = FetchType.LAZY)
        var EmployeeCooperations: MutableList<Cooperation> = mutableListOf(),

        @OneToMany(mappedBy = "organizationalSupervisorEmployee", cascade = [CascadeType.ALL], orphanRemoval = true, fetch = FetchType.LAZY)
        var EmployeeOrganizationalSupervisorArrivals: MutableList<Arrival> = mutableListOf(),

        @OneToMany(mappedBy = "researchTutorEmployee", cascade = [CascadeType.ALL], orphanRemoval = true, fetch = FetchType.LAZY)
