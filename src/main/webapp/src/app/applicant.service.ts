...
export class ApplicantService {
  ...
  getApplicantByPeselAndPrgCode(pesel: string, prgCode: string) {
    return this.http.get<Page>(`${this.apiUrl}/?pesel=${pesel}&prg_code=${prgCode}`)
  }

  getApplicantBySurnameAndPrgCode(surname: string, prgCode: string) {
    return this.http.get<Page>(`${this.apiUrl}/?surname=${surname}&prg_code=${prgCode}`)
  }

  getApplicantByEmailAndPrgCode(email: string, prgCode: string) {
    return this.http.get<Page>(`${this.apiUrl}/?email=${email}&prg_code=${prgCode}`)
  }

  getApplicantByPesel(pesel: string) {
    return this.http.get<Page>(`${this.apiUrl}/?pesel=${pesel}`)
  }

  getApplicantBySurname(surname: string) {
    return this.http.get<Page>(`${this.apiUrl}/?surname=${surname}`)
  }

  getApplicantByEmail(email: string) {
    return this.http.get<Page>(`${this.apiUrl}/?email=${email}`)
  }

  getApplicantByValues(valueType: string, value: string) {
    return this.http.get<Page>(`${this.apiUrl}/?${valueType}=${value}`)
  }
